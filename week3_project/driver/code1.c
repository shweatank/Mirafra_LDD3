...
MODULE_DESCRIPTION("Virtual Keyboard Logger with Non-Destructive Read");
...
static ssize_t virt_read(struct file *filp, char __user *buf, size_t count, loff_t *off)
{
    ...
    for (i = buf_tail; i < buf_head && len < sizeof(tmp) - 1; i++) {
        ...
        len += snprintf(tmp + len, sizeof(tmp) - len,
                        "Key: %c | Time: %lld.%09lu\n",
                        e->key,
                        (long long)e->ts.tv_sec,
                        e->ts.tv_nsec);
    }

    data_available = 0;  // clear availability flag; keep buffer
...
static int proc_show(struct seq_file *m, void *v)
{
    ...
    for (i = buf_tail; i < buf_head; i++) {
        struct key_event *e = &key_log[i % MAX_KEYS];
        seq_printf(m, "Key: %c | Time: %lld.%09lu\n",
                   e->key, (long long)e->ts.tv_sec, e->ts.tv_nsec);
    }
    return 0;
}
