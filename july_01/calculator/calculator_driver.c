...
static ssize_t dev_write(struct file *file, const char __user *user_buf, size_t count, loff_t *ppos)
{
    ...
    if (copy_from_user(&calc, user_buf, sizeof(calc)))
        return -EFAULT;

    switch (calc.choice) {
        case 1: calc.result = calc.op1 + calc.op2; break;
        case 2: calc.result = calc.op1 - calc.op2; break;
        case 3: calc.result = calc.op1 * calc.op2; break;
        case 4:
            if (calc.op2 == 0)
                calc.result = 0;
            else
    ...
    printk(KERN_INFO "simple_char_dev: result = %d\n", calc.result);
...
static ssize_t dev_read(struct file *file, char __user *user_buf, size_t count, loff_t *ppos)
{
    ...
    if (copy_to_user(user_buf, &calc, sizeof(calc)))
        return -EFAULT;
