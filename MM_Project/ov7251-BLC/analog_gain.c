...
/* === NEW: helper to set analogue gain via V4L2 control === */
static int set_analog_gain(int fd, int gain) {
    ...
    if (ioctl(fd, VIDIOC_S_CTRL, &ctrl) < 0) {
        perror("VIDIOC_S_CTRL (ANALOG GAIN)");
        return -1;
    }
    printf("Analogue Gain set to %d\n", gain);
    return 0;
}
...
int main() {
    ...
    uint8_t *u_plane = malloc((WIDTH/2) * (HEIGHT/2));
    uint8_t *v_plane = malloc((WIDTH/2) * (HEIGHT/2));

    if (!y_plane || !u_plane || !v_plane) {
        fprintf(stderr, "Failed to allocate YUV planes\n");
        return 1;
    }

    // U and V constant (neutral gray chroma)
    ...
    int hflip = 0, vflip = 0;
    int analogue_gain = 0x100;   // === NEW: Default analogue gain ===
    ...
    while (running) {
        ...
        // Apply brightness/contrast/flip → write into Y plane
        for (int y = 0; y < HEIGHT; y++) {
            for (int x = 0; x < WIDTH; x++) {
                ...
                y_plane[y * WIDTH + x] = clamp(newval);
            }
        }

        // Update SDL YUV texture
        ...
        while (SDL_PollEvent(&e)) {
            if (e.type == SDL_QUIT) running = 0;
                ...
                switch (e.key.keysym.sym) {
                    ...
                    /* === NEW: Analogue gain control === */
                    case SDLK_g:     
                        analogue_gain += 0x10;
                        if (analogue_gain > 0x3FF) analogue_gain = 0x3FF;
                        set_analog_gain(fd, analogue_gain);
                        break;

                    case SDLK_f:
                        analogue_gain -= 0x10;
                        if (analogue_gain < 0x00) analogue_gain = 0x00;
                        set_analog_gain(fd, analogue_gain);
                        break;
                }
            }
