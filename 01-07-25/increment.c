...
static ssize_t dev_read(struct file *file, char __user *user_buffer, size_t len, loff_t *offset) 
{
    ...
    char buf[10];
    printk(KERN_INFO "Simple_char_dev:Read %s\n",buf);
    return bytes_read;
}
...
static ssize_t dev_write(struct file *file, const char __user *user_buffer, size_t len, loff_t *offset) 
{
    ...
    device_buffer[len]='\0';
    kstrtoint(device_buffer,sizeof(device_buffer),&num);
    printk(KERN_INFO "simple_char_dev:Wrote %d \n", num++);
    snprintf(device_buffer,sizeof(device_buffer),"%d",num);
    return len;
}
