...
static ssize_t dev_read(struct file *file, char __user *user_buf, size_t count, loff_t *ppos)
{
    ...
    *ppos += count;
    printk(KERN_INFO "read_write_char_dev: Read %zu bytes\n", count);
    return count;
}
...
static ssize_t dev_write(struct file *file, const char __user *user_buf, size_t count, loff_t *ppos)
{
    ...
    device_buffer[count] = '\0';  // Null terminate
    to_rev(device_buffer, strlen(device_buffer));  // Reverse string
    printk(KERN_INFO "read_write_char_dev: Written %zu bytes (reversed)\n", count);
    return count;
}
...
static int __init simple_char_init(void)
{
    ...
    if (major < 0)
    {
        printk(KERN_ALERT "read_write_char_dev: failed to register character device\n");
        return major;
    }
...
MODULE_DESCRIPTION("Minimal character device driver that reverses string");
