...
#define UART0_BASE 0xFE201000  // PL011 UART base (Raspberry Pi 4)
#define UART_DR    0x00
#define UART_FR    0x18
...
static int uart_rx_thread(void *data) {
    ...
    while (!kthread_should_stop()) {
        if (!(readl(uart_base + UART_FR) & (1 << 4))) { // RX FIFO not empty
            c = uart_recv_char();
            last_rx_char = c;
...
static ssize_t uart_write(struct file *file, const char __user *buf, size_t count, loff_t *ppos) {
    ...
    kbuf[count] = '\0';  // Null-terminate the input

    // Parse: <num1> <num2> <op>
    ret = sscanf(kbuf, "%d %d %c", &num1, &num2, &op);
    if (ret != 3) {
        uart_send_string("Error: Invalid format. Use: num1 num2 op\n");
        return count;
    }
    ...
    switch (op) {
        ...
        case '/':
            if (num2 == 0) {
                uart_send_string("Error: Divide by zero\n");
                return count;
            }
            result = num1 / num2;
            break;
        default:
            uart_send_string("Error: Use operators + - * /\n");
            return count;
    }

    snprintf(outstr, BUF_SIZE, "Result: %d %c %d = %d\n", num1, op, num2, result);
    uart_send_string(outstr);
...
static int __init uart_init(void) {
    pr_info("UART Arithmetic Driver Init\n");
    ...
    writel((3 << 5) | (1 << 4), uart_base + UART_LCRH); // 8N1, FIFO enabled
    writel(0x0, uart_base + UART_IMSC);
    writel((1 << 0) | (1 << 8) | (1 << 9), uart_base + UART_CR); // UARTEN, TXE, RXE
    ...
    uart_send_string("UART arithmetic driver loaded.\n");
...
static void __exit uart_exit(void) {
    ...
    pr_info("UART Arithmetic Driver Exit\n");
}
...
MODULE_DESCRIPTION("UART driver with dynamic major, arithmetic ops via /dev and UART output");
