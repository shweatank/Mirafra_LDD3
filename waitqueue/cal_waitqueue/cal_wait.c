...
MODULE_DESCRIPTION("Calculator using Wait Queue");
...
static int flag = 0;
static char result_buf[100];  // Result stored here

// --- Reader waits for result ---
static ssize_t my_read(struct file *filp, char __user *buf, size_t len, loff_t *off)
{
	 if (*off > 0) return 0;
    printk(KERN_INFO "mycalc: Reader sleeping...\n");
    wait_event_interruptible(wq, flag != 0);  // Sleep until writer wakes up

    flag = 0;

    if (copy_to_user(buf, result_buf, strlen(result_buf)))
        return -EFAULT;
     *off += strlen(result_buf); 

    return strlen(result_buf);
}

// --- Writer performs calculation and wakes reader ---
...

{
    ...
    if (sscanf(kbuf, "%d %c %d", &a, &op, &b) != 3) {
        ...
        case '/': 
            if (b == 0)
                snprintf(result_buf, sizeof(result_buf), "Divide by zero\n");
            else
                res = a / b;
            break;
        default:
