...
{
        struct i2c_client *client = v4l2_get_subdevdata(&ov7251->subdev);
	...
	ret = reg_write(client, OV7251_PRE_ISP_00, val);
...
static int ov7251_s_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_GAIN:
		...
		if (ctrl->val > 0xFF) 
			ctrl->val = 0xFF;
		priv->analog_gain = ctrl->val;
		ret = reg_write(client, OV7251_AEC_AGC_ADJ_1, ctrl->val & 0xFF); 
		
		return ret;

	case V4L2_CID_BLACK_LEVEL: 
		{
			u8 bl_val = ctrl->val & 0xFF;
			if(bl_val>63)
			{
			    bl_val = 63;
			}
			...
			dev_info(&client->dev, "BLACK LEVEL = %d\n", bl_val & (~0x80));
			if((bl_val&(~0x80)) == 63)
			{
			    dev_info(&client->dev, "BLACK LEVEL Disabled\n");
			    ret = reg_write(client, 0x4000, 0x00);
			    if (ret)
				return ret; 
			}
			else
			{
		 	    ret = reg_write(client, 0x4000, 0x01);
			    if (ret)
				return ret;
			    ret = reg_write(client, 0x4001, bl_val);
			}
       	...
       	case V4L2_CID_TEST_PATTERN:
                ret = ov7251_set_test_pattern(priv, ctrl->val);
                return ret;
...
static int ov7251_ctrls_init(struct v4l2_subdev *sd)
{
	...
	v4l2_ctrl_new_std(&priv->ctrl_handler, &ov7251_ctrl_ops,
        ...
        v4l2_ctrl_new_std(&priv->ctrl_handler, &ov7251_ctrl_ops,
                      V4L2_CID_ANALOGUE_GAIN, 0, 0xFF, 1, 0x80);
	v4l2_ctrl_new_std(&priv->ctrl_handler, &ov7251_ctrl_ops,
                  V4L2_CID_BLACK_LEVEL, 0, 63, 1, 16);
        
	v4l2_ctrl_new_std_menu_items(&priv->ctrl_handler, &ov7251_ctrl_ops,
		...
		
{
	...
	if (!priv)
		return -ENOMEM;
	 /* Initialize pre_isp_00 (test pattern register) */
	priv->pre_isp_00 = 0x00;
	reg_write(client, OV7251_PRE_ISP_00, 0x00);  /* Disable test pattern by default */
