...
static ssize_t dev_read(struct file *file, char __user *user_buffer, size_t len, loff_t *offset) 
{
    ...
    if (copy_to_user(user_buffer, &i , sizeof(int)))
    {
        return -EFAULT;
    }
    printk(KERN_INFO "send to user : %d", i);
    return sizeof(int);
}
...
static ssize_t dev_write(struct file *file, const char __user *user_buffer, size_t len, loff_t *offset) 
{
 //   if(len>BUF_SIZE-1)
 //	    len=BUF_SIZE-1;
    if (copy_from_user(&i, user_buffer,sizeof(int))) 
    {
        return -EFAULT;
    }
    printk(KERN_INFO "The value from user is %d\n",i);
    i++;
