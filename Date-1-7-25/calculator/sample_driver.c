...
static ssize_t dev_write(struct file *file, const char __user *user_buffer, size_t len, loff_t *offset) 
{
    ...
    if(len>BUF_SIZE-1)
	    len=BUF_SIZE-1;
    if (copy_from_user(in_buf, user_buffer,len)) 
    {
        return -EFAULT;
    }
    in_buf[len]='\0';

    sscanf(in_buf,"%d%c%d",&a,&op,&b);
    switch (op)
    {
	    case '+':c=a+b;break;
	    case '-':c=a-b;break;
	    case '*':c=a*b;break;
	    case '/':c=a/b;break;
	    default:printk(KERN_INFO "Invaild Info");
    }
    snprintf(device_buffer, BUF_SIZE, "%d%c%d=%d", a, op, b, c);
