...
// Resize image to 128x32 using nearest neighbor
void resize_image(unsigned char *src, int src_w, int src_h, int channels, unsigned char *dst) {
    ...
    for (int y = 0; y < HEIGHT; y++) {
        for (int x = 0; x < WIDTH; x++) {
            ...
            dst[(y * WIDTH + x) * channels + 0] = src[src_idx + 0];
            dst[(y * WIDTH + x) * channels + 1] = src[src_idx + 1];
            dst[(y * WIDTH + x) * channels + 2] = src[src_idx + 2];
        }
    }
}
...
int main(int argc, char *argv[]) {
    if (argc < 2) {
        fprintf(stderr, "Usage: %s image.png|jpg|bmp\n", argv[0]);
        return 1;
    }
    ...
    resize_image(img, w, h, 3, scaled_rgb);
