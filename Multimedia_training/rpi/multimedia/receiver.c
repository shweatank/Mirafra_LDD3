...
int oled_init(int fd) {
    uint8_t cmds[] = {
        ...
        0xA1,              // Segment re-map
        0xA6,              // Normal display
        0xA8, 0x1F,        // Multiplex ratio: 31 (for 32px height)
...
// Convert RGB → OLED buffer with scaling
...
int main(int argc, char **argv) {
    if (argc < 2) {
        fprintf(stderr, "Usage: %s <uart_dev>\n", argv[0]);
        return 1;
    }
    ...
    // Read size first
    uint32_t size;
    if (read(uart_fd, &size, sizeof(size)) != sizeof(size)) {
