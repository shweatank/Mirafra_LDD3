                        ...
                        size_t len, loff_t *off)
{
    ...
    /*copy_from_user(expression,buf,len);
    expression[len]='\0';*/
    if (len >= sizeof(expression))
    return -EINVAL;

if (copy_from_user(expression, buf, len))
    return -EFAULT;

expression[len] = '\0';  // Null-terminate

    flag = 1;
    wake_up_interruptible(&wq);
    sscanf(expression,"%d %c %d",&n1,&op,&n2);
    switch(op)
    {
	    case '+':result=n1+n2;
		     break;
	    ...
	    case '/':result = n1/n2;
		     break;
	    default:printk(KERN_INFO"Invalid operation\n");
    }
