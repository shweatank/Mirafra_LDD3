...
// Add this before simplefs_fill_super()
...
root_inode->__i_ctime = now;
root_inode->__i_mtime = now;
root_inode->__i_atime = now;
...
struct timespec64 now2 = current_time(file_inode);
file_inode->__i_ctime = now2;
file_inode->__i_mtime = now2;
file_inode->__i_atime = now2;
