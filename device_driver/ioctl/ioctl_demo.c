...
static long my_ioctl(struct file *file,unsigned int cmd,unsigned long arg)
{
	...
	switch(cmd)
	{
		case IOCTL_SET_NUM:if(copy_from_user(&user_val,(int __user *)arg,sizeof(user_val)))
					   return -EFAULT;
				   device_number =user_val;
				   pr_info("ioctl demo :Received number:%d\n",device_number);
				   break;
		case IOCTL_GET_NUM:if(copy_to_user((int __user*)arg, &device_number,sizeof(device_number)))
					   return -EFAULT;
...
static int __init ioctl_init(void)
{
    ...
    if (ret < 0) {
        pr_err("Failed to register char device\n");
        return ret;  // important to return error here
    }
