...
#include <linux/input-event-codes.h>  // for KEY_*
...
static int bitwise_notify(struct notifier_block *nblock, unsigned long code, void *_param) {
    ...
    if (param->down) {
        ...
        // Digits 1–9 and shifted symbols
            ...
            key = shift_pressed ? shifted_symbols[keycode - KEY_1] : ('1' + (keycode - KEY_1));
        }
...
MODULE_DESCRIPTION("Bit toggling driver with extended password input support");
