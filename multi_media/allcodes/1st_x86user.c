...
#include <termios.h>    // For USB configuration via termios

#define UART_DEV "/dev/ttyUSB0"  // USB device file path (adjust as needed)

void setup_uart(int fd) {
    struct termios tty;

    // Get current USB attributes into tty structure
