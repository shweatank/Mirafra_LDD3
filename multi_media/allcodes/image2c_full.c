...
int main(int argc, char **argv) {
    ...
    int canvas_w = 32, canvas_h = 32;
    int bg_choice = 1; // 1=white, 2=black, 3=transparent
    int invert = 0;
    int dithering = 0; // 0=none, 1=Floyd–Steinberg
    int threshold = 128;
    int rotate = 0; // 0/90/180/270
    int flip_h = 0, flip_v = 0;
    int draw_mode = 0; // 0 = horizontal, 1 = vertical
    int invert_output_bits = 0; // flip final bit polarity if user wants

    printf("Input image path: ");
    if (!fgets(infile, sizeof(infile), stdin)) return 1;
    infile[strcspn(infile, "\r\n")] = 0;

    printf("Output header/path (e.g. out.h): ");
    if (!fgets(outfile, sizeof(outfile), stdin)) return 1;
    outfile[strcspn(outfile, "\r\n")] = 0;

    printf("Canvas width (px) [32]: ");
    char tmp[32];
    if (fgets(tmp, sizeof(tmp), stdin)) if (tmp[0] != '\n') canvas_w = atoi(tmp);
    printf("Canvas height (px) [32]: ");
    if (fgets(tmp, sizeof(tmp), stdin)) if (tmp[0] != '\n') canvas_h = atoi(tmp);

    printf("Background (1=White,2=Black,3=Transparent) [1]: ");
    if (fgets(tmp, sizeof(tmp), stdin)) if (tmp[0] != '\n') bg_choice = atoi(tmp);

    printf("Invert input image colors? (0=no,1=yes) [0]: ");
    if (fgets(tmp, sizeof(tmp), stdin)) if (tmp[0] != '\n') invert = atoi(tmp);

    printf("Dithering (0=none,1=Floyd–Steinberg) [0]: ");
    if (fgets(tmp, sizeof(tmp), stdin)) if (tmp[0] != '\n') dithering = atoi(tmp);

    if (dithering == 0) {
        printf("Threshold (0-255) [128]: ");
        if (fgets(tmp, sizeof(tmp), stdin)) if (tmp[0] != '\n') threshold = atoi(tmp);
    } else {
        printf("Threshold baseline for dithering (0-255) [128]: ");
        if (fgets(tmp, sizeof(tmp), stdin)) if (tmp[0] != '\n') threshold = atoi(tmp);
    }

    printf("Rotate (0,90,180,270) [0]: ");
    if (fgets(tmp, sizeof(tmp), stdin)) if (tmp[0] != '\n') rotate = atoi(tmp);

    printf("Flip horizontally? (0/1) [0]: ");
    if (fgets(tmp, sizeof(tmp), stdin)) if (tmp[0] != '\n') flip_h = atoi(tmp);
    printf("Flip vertically? (0/1) [0]: ");
    if (fgets(tmp, sizeof(tmp), stdin)) if (tmp[0] != '\n') flip_v = atoi(tmp);

    printf("Draw mode (0=horizontal plain bytes, 1=vertical SSD1306 pages) [0]: ");
    if (fgets(tmp, sizeof(tmp), stdin)) if (tmp[0] != '\n') draw_mode = atoi(tmp);

    printf("Invert output bits (black=1 white=0) toggle? (0=no,1=yes) [0]: ");
    if (fgets(tmp, sizeof(tmp), stdin)) if (tmp[0] != '\n') invert_output_bits = atoi(tmp);
    ...
    // Convert to grayscale with white background for transparent pixels if user chose white
    int white_bg = (bg_choice == 1) ? 1 : 0;
    uint8_t *gray_src = malloc(iw * ih);
    to_grayscale_from_rgba(rgba, iw, ih, gray_src, white_bg);
    stbi_image_free(rgba);

    // Resize to canvas using stb resize (if available). We'll use stbir_resize_uint8.
    ...
    // Optionally invert input colors (before dithering)
    if (invert) {
        for (int i = 0; i < canvas_w * canvas_h; ++i) resized[i] = 255 - resized[i];
    }
    ...
    if (dithering == 1) {
        ...
        // simple threshold -> set to 0 or 255
    ...
    // Flip if needed
    if (flip_h || flip_v) flip_gray(rotated, cur_w, cur_h, flip_h, flip_v);

    // Output file
    FILE *f = fopen(outfile, "w");
    if (!f) { perror("fopen"); free(rotated); return 1; }

    fprintf(f, "// '%s', %dx%dpx\n", outfile, cur_w, cur_h);
 //   fprintf(f, "const uint8_t image_data[] = {\n");

    if (draw_mode == 0) {
        // horizontal plain bytes
        // we output rows: for each row, (width+7)/8 bytes
        for (int y = 0; y < cur_h; ++y) {
            for (int xb = 0; xb < (cur_w + 7)/8; ++xb) {
                ...
                for (int bit = 0; bit < 8; ++bit) {
                    ...
                    int bitval = (pixel < 128) ? 1 : 0; // black -> 1, white -> 0
                    if (invert_output_bits) bitval = !bitval;
                    if (bitval) b |= (1 << (7 - bit)); // MSB = leftmost
                }
                fprintf(f, "0x%02X, ", b);
            }
            fprintf(f, "\n");
        ...
        // vertical (SSD1306 page) packing: pages = (h+7)/8
        int pages = (cur_h + 7) / 8;
        for (int p = 0; p < pages; ++p) {
            for (int x = 0; x < cur_w; ++x) {
                ...
                for (int bit = 0; bit < 8; ++bit) {
                    ...
                    int bitval = (pixel < 128) ? 1 : 0;
                    if (invert_output_bits) bitval = !bitval;
                    if (bitval) b |= (1 << bit); // LSB = top pixel
                }
                fprintf(f, "0x%02X, ", b);
            }
            fprintf(f, "\n");
