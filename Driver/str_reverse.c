...
#define DEVICE_NAME "str_reverse"
#define BUF_LEN 128
...
{
        printk(KERN_INFO "str_reverse: Device Opened\n");
        return 0;
}
...
static ssize_t dev_read(struct file *file,char __user *user_buf,size_t count,loff_t *ppos)
{
        ...
        printk(KERN_INFO "str_reverse: Read %d bytes\n",bytes_read);
        return bytes_read;
}
...
static ssize_t dev_write(struct file *file, const char __user *user_buf,size_t count,loff_t *ppos)
{
        ...
        printk(KERN_INFO "str_reverse: Written %zu bytes\n",count);
	reverse(device_buffer,count);
        return count;
}
...
static int __init simple_char_init(void)
{
        ...
        printk(KERN_INFO "str_reverse: Registered with major number %d\n",major);
        return 0;
}
...
static void __exit simple_char_exit(void)
{
        ...
        printk(KERN_INFO "str_reverse: Unregistered device\n");
}
