...
/*Custom changes: adding framerate control*/

static int ov7251_write_seq_regs(struct i2c_client *client, u16 reg, u8 *val,
                                 u8 num)
{
        u8 regbuf[5];
        ...
        i2c_master_send(client, regbuf, nregbuf);
        if (ret < 0) {
                return ret;
        }
...
static int ov7251_vts_configure(struct ov7251 *ov7251, struct i2c_client *client, s32 vblank)
{
        u8 vts[2];

        vts[0] = ((ov7251->cur_mode->height + vblank) & 0xff00) >> 8;
        vts[1] = ((ov7251->cur_mode->height + vblank) & 0x00ff);

        return ov7251_write_seq_regs(client, OV7251_VTS_HIGH, vts, 2);
}
...
/*Custom changes: adding framerate control*/
	...
	case V4L2_CID_VBLANK:
                ret = ov7251_vts_configure(priv, client, ctrl->val);
                break;
...
/*Custom changes: adding framerate control*/
	...
	vblank_max = OV7251_VTS_MAX - priv->cur_mode->height;
        vblank_def = priv->cur_mode->vts_def - priv->cur_mode->height;
        v4l2_ctrl_new_std(&priv->ctrl_handler, &ov7251_ctrl_ops, V4L2_CID_VBLANK, OV7251_VTS_MIN_OFFSET, vblank_max, 1, vblank_def);
...
static int ov7251_detect(struct i2c_client *client, struct i2c_board_info *info)
{
	...
	strncpy(info->type, "ov7251", I2C_NAME_SIZE);
